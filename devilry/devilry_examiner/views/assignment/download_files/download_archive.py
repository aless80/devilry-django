...
#ALE 
class CompressedAssignmentFileDownloadViewALE(generic.TemplateView):
    def get(self, request, *args, **kwargs):
        assignment_id = kwargs.get('assignment_id')
        assignment = get_object_or_404(core_models.Assignment, id=assignment_id)
...
class App(crapp.App):
    appurls = [
        ...
        #ALE 
        crapp.Url(
            r'^assignment-file-download-ALE/(?P<assignment_id>[0-9]+)$',
            CompressedAssignmentFileDownloadViewALE.as_view(),
            name='assignment-file-download-ALE'),
        crapp.Url(
            r'assignment-download-ALE-api/(?P<content_object_id>[0-9]+)$',
            batch_download_api.BatchCompressionAPIAssignmentView.as_view(),
            name='assignment-file-download-ALE-api'
        )
